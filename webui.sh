...
# 0.0.0.0 only means "listen on all interfaces" and is not a valid browser
# address. Opening http://0.0.0.0:8501 on macOS/Linux may go through a proxy
# or gateway and fail with 502, so bind 127.0.0.1 by default, matching the
...
# Probe ports with Python to avoid depending on lsof/nc across macOS/Linux.
